...
isHit (b1x, b1y) (b2x, b2y) =
  (b1x - 10) < b2x + 32 &&
  b1x + 50 - 10 > b2x && b1y < b2y + 32 && b1y + 54 > b2y

makeRow ∷ String → Int → Level
makeRow row y =
  [ ( ( (fromIntegral x * 32) - ((1024 / 2) - 16)
      , (fromIntegral y * 32) - ((768 / 2) - 16))
    , row !! x)
  | x ← [0 .. length row - 1]
  , row !! x == '*' || row !! x == '%'
  ]
...
getY ∷ GameState → Float
getY gs = snd (position gs)

getX ∷ GameState → Float
getX gs = fst (position gs)
...
handleKeys (EventKey (SpecialKey KeySpace) Down _ _) gs =
  gs
    { speedY =
        if isCollision gs (getX gs, getY gs + speedY gs) '*'
          then 6
          else (-6)
    }
...
moveX East gs =
  if not (isCollision gs (getX gs + speedX gs, getY gs) '*')
    then (getX gs + speedX gs, getY gs)
    else position gs
moveX West gs =
  if not (isCollision gs (getX gs + speedX gs * (-1), getY gs) '*')
    then (getX gs + speedX gs * (-1), getY gs)
    else position gs
